...
<?php
 ...
 $sql = mysqli_query($db, "SELECT id_kar, SUM(kebersihan) as bersih, SUM(kerapian) as kerap, SUM(pelayanan) as pelayan, SUM(keramahan) as ramah, tgl FROM `ratingkami` WHERE tgl = '$date' GROUP BY id_kar ");
 //Buat tabel untuk menampilkan hasil
 echo "<h2 class='text-center'>MATRIK AWAL</h2>
 ...
 while ($dt = mysqli_fetch_array($sql)) {
  $jumlah= ($dt['bersih'])+($dt['kerap'])+($dt['pelayan'])+($dt['ramah']);
  echo "<tr>
   <td>$no</td>
 ...
 $sql2 = mysqli_query($db, "SELECT id_kar, SUM(kebersihan) as bersih, SUM(kerapian) as kerap, SUM(pelayanan) as pelayan, SUM(keramahan) as ramah, tgl FROM `ratingkami` WHERE tgl = '$date' GROUP BY id_kar");
 //Buat tabel untuk menampilkan hasil
 echo "<h2 class='text-center'>NORMALISASI</h2>
 <table class='table table-bordered table-hover table-light' width=500 style='border:1px; #ddd; solid; border-collapse:collapse' border=1>
  <tr>
   <th>No</th>
   <th>Nama</th>
 ...
 while ($dt2 = mysqli_fetch_array($sql2)) {
  echo "<tr>
   ...
   <td>".round($dt2['bersih']/$max['maxK1'],2)."</td>
   <td>".round($dt2['kerap']/$max['maxK2'],2)."</td>
   <td>".round($dt2['pelayan']/$max['maxK3'],2)."</td>
   <td>".round($dt2['ramah']/$max['maxK4'],2)."</td>
  </tr>";
 $no++;
 }
 echo "</table>";
 ...
 //Buat tabel untuk menampilkan hasil
 echo "<h2 class='text-center'>PERANGKINGAN</h2>
 <table class='table table-bordered table-hover table-light' width=500 style='border:1px; #ddd; solid; border-collapse:collapse' border=1>
  <tr>
   <th>no</th>
   <th>Nama</th>
